 ...
 */
public record SmsRequest(String phoneNumber, String certificationNumber) {

    //정적 팩토리 메서드: sms 전송을 위한 요청 객체 생성
    public static SmsRequest createForSending(String phoneNumber){
        if(phoneNumber == null || phoneNumber.isBlank()){
            throw new IllegalArgumentException("핸드폰번호는 빈칸일 수 없습니다");
        }
        return new SmsRequest(phoneNumber, null);
    }
    ...
    public static SmsRequest createForVerification(String phoneNumber, String certificationNumber){
        ...
        return new SmsRequest(phoneNumber, certificationNumber);
    }
}
